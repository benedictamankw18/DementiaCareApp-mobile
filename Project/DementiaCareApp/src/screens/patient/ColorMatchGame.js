 ...
 * 
 * Color matching game for visual skills and recognition
 */
...
const ColorMatchGame = ({ route, navigation }) => {
  const { getTextSize, settings } = useSettings();
  const difficulty = route?.params?.difficulty || 'easy';
  const gameConfig = COLOR_SETS[difficulty];
  ...
  useEffect(() => {
    if (matched.length === gameConfig.pairsCount * 2 && matched.length > 0) {
      setGameWon(true);
  ...
  const initializeGame = () => {
    ...
    const gameCards = [];
    const colorsToUse = gameConfig.colors;

    for (let i = 0; i < gameConfig.pairsCount; i++) {
      gameCards.push(
        { id: i * 2, color: colorsToUse[i], pairId: i },
        { id: i * 2 + 1, color: colorsToUse[i], pairId: i }
      );
    }
    ...
    setGameOver(false);
    setGameWon(false);
    console.log('[ColorMatchGame] Game initialized with', gameConfig.pairsCount, 'color pairs');
  };
  ...
  const handleCardPress = (cardId) => {
    ...
    if (newFlipped.length === 2) {
      ...
      if (card1.pairId === card2.pairId) {
        ...
        console.log('[ColorMatchGame] Match found! Color:', card1.color);
      } else {
  ...
  const saveGameSession = async (won = false) => {
    try {
      ...
      await logGameSession(user.uid, 5, sessionData);
      console.log('[ColorMatchGame] Session saved:', sessionData);
    } catch (error) {
      console.error('[ColorMatchGame] Error saving session:', error);
    }
  };
  ...
  const progressPercentage = matched.length / (gameConfig.pairsCount * 2);
  const cardSize = Dimensions.get('window').width / 4 - spacing.lg;
    ...
    <ScrollView style={[styles.container, settings.highContrast && { backgroundColor: '#000' }]}>
      ...
      <View style={[styles.progressSection, settings.highContrast && { backgroundColor: '#1a1a1a', borderBottomWidth: 1, borderBottomColor: '#FFF' }]}>
        <View style={styles.progressHeader}>
          <Text style={[styles.progressLabel, { fontSize: getTextSize(14), color: colors.text }, settings.highContrast && { color: '#FFF' }]}>
            Pairs Found: {matched.length / 2}/{gameConfig.pairsCount}
          </Text>
        </View>
        <ProgressBar
            ...
            <TouchableOpacity
              ...
              style={[
                ...
                flipped.includes(card.id) || matched.includes(card.id)
                  ? [styles.cardFlipped, { backgroundColor: card.color }]
      ...
      {gameOver && !gameWon && (
        <View style={[styles.resultScreen, settings.highContrast && { backgroundColor: '#1a1a1a' }]}>
          ...
          <Text style={[styles.resultSubtitle, { fontSize: getTextSize(16), color: colors.text }, settings.highContrast && { color: '#FFF' }]}>
            You matched {matched.length / 2}/{gameConfig.pairsCount} color pairs
          </Text>
          <View style={styles.resultStats}>
            <View style={styles.resultStatItem}>
              <Text style={[styles.resultStatLabel, { fontSize: getTextSize(14), color: colors.gray }, settings.highContrast && { color: '#FFF' }]}>Score</Text>
